...
## Pass 46K — diamond specialist larger-N confirmation + reference gap audit (LOCAL-ONLY, NOT Kaggle) — decision: diamond_specialist_inconclusive_needs_more_n
- **Strategic test:** take 46J's `promising_local_only` planner and try to *confirm* it at larger N on a pre-registered, frozen ladder — does the parent edge survive, does it separate from the *stronger* generic `option_value` scorer, and is the measurement apparatus itself clean — **reusing the 46J/46H tarballs byte-for-byte** (no rebuild).
- **What the evidence says — the edge confirmed and became attributable; the apparatus did not.** Practical vs parent held and tightened (61/84 = 0.726, Wilson [0.623, 0.810]); the attribution H2H that was only directional in 46J **cleared** (53/84 = 0.631, lower 0.524 > 0.5) with a significant Fisher increment over both generic-vs-parent rates — the parent edge is the planner's, not the 46H generic family's. **But** the triggered `spec_vs_spec` self-mirror, which must be ~0.5 by symmetry, sat persistently at 0.324 (CI excludes 0.5) across both tranches and both seats.
- **Durable lesson:** a clean, attributable edge is **still not a confirmation** when a same-agent noise control is itself biased — confirming through an unclean apparatus would measure the apparatus, not the planner. Pre-register the noise-clean rule and **honor it**: when the control fails, return *inconclusive*, don't move the goalposts. The likely culprit (a cabt seat-order asymmetry) is itself the next thing to fix, not to explain away.
- **Decision `diamond_specialist_inconclusive_needs_more_n`** — provably independent of references. LOCAL-ONLY; public references benchmark-only & excluded (pooled 1/60 = 0.017 distance-from-parity, parity unsupported); no upload/promote/register/tick/republish/redeploy; all five workflows stayed not-started; shared site not regenerated. Next: re-run the `spec_vs_spec` mirror **seat-paired** at N≥60 to diagnose the bias — the practical + attribution arms already clear, so a clean mirror puts `confirmed_local_candidate` in reach with no new build.

Detail: `data/reports/pass46k_diamond_specialist_confirmation_report.md`; `data/experiments/pass46k_*`.

## Pass 46L — diamond reference-gap planner upgrade v1 (LOCAL-ONLY, NOT Kaggle) — decision: diamond_v1_not_promising
- **Strategic test:** can ONE owned upgrade `cg_typed_diamond_specialist_planner_v1`, built from v0 via a narrow VISIBLE-ONLY structural diff, materially close the 46K public-reference gap (v0 = W1/L59 pooled) — judged by a PRE-REGISTERED pooled reference-improvement test against the frozen 1/59 baseline?
- **What the evidence says — v1 is well-formed but INERT.** A stage-2 blocking gate replays v0 vs v1 on the SAME real frames first: **0 / 312** decisions changed (rate 0.0; 0 illegal). The levers (attach-to-active weight, already-energized penalty, promote tilt) shift the attach crossover only fractionally and v0 already prefers the active attacker, so the argmax never moves. An inert v1 is behaviorally v0, so the frozen ladder blocks all eval/reference panels rather than spend 300+ games reproducing W1/L59.
- **Durable lesson:** the public-reference gap is a **card-id GROUNDING gap, not a thin-policy gap**. 46K (role-keyed visible layer, fired too rarely) and 46L (structural visible layer, outright inert) together prove a second visible-only policy layer is a dead end. The missing lever — *which* card is the right attacker / energy target — is not visible-only; it needs grounded, authoritative card identity (NOT extended from observed play, which would be inventing IDs).
- **Decision `diamond_v1_not_promising`** — provably independent of references (the inert gate short-circuits before any reference value is consulted). LOCAL-ONLY; public references benchmark-only & never gated; no upload/promote/register/tick/republish/redeploy; all five workflows stayed not-started; shared site not regenerated. Next: a card-id grounding pass against an audited owned card database, or a different archetype — NOT another visible-only layer.
