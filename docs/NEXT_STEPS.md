...
## Pass 46J — Diamond cg_typed specialist turn-planner v0 (LOCAL-ONLY, NO redeploy) — decision: diamond_specialist_promising_local_only
- Built ONE owned `cg_typed` turn **PLANNER** (not a flat scorer) for internal parent `diamond_toolbox_diancie`: `src/ptcg_activegraph/analysis/diamond_specialist.py` with `DiamondBoardView`/`DiamondRoleMap`/`DiamondTurnPlan` + **11 per-context policies** dispatched by raw cg option-type code. Deck byte-copied from the parent (sha `6bd17785…`, policy-only); `main.py` owned & original, inlining the planner byte-identically (`INLINE_DIAMOND_SPECIALIST_V0`, parity-verified). Pure/read-only: no ObjectStorage/EventStore/cg/online-Search/file-IO; never-raise → legal fallback.
- **Audit (Part B):** every deck ID verified vs `EN_Card_Data.csv` (no invented IDs); 60 legal / 15 unique; 17 Pokémon (all Basic), 27 energy, 16 trainer (18 search, 4 draw); main attacker **766 Mega Diancie ex**. Roles from verified metadata; heuristic groupings flagged `roles_inferred`.
- **Safe/runnable/non-inert:** Part-A preflight `all_ok`; cg_typed lane accepts / stdlib rejects (bytes unchanged); 14 fixtures clean (legal indices, claims honest, no hidden zone); smoke 10/10 (0 timeout/error). Non-inertness vs parent 17/38 = 44.7% (0 illegal/0 exception), **94.1% plan-field-driven** (`search_targets`/`setup_bench`/`safe_discard`); distinguishable from the 46H generic option_value scorer (15/38).
- **Eval panel (pre-registered, 2/229 invalid):** **practical** specialist vs parent **32/43 = 0.744**, Wilson95 [0.598, 0.851] (lower>0.5), clean seats → `confirmed_edge`. The two generic 46H scorers do NOT beat the same parent (floor 0.485, option_value 0.394). **Attribution** holds: one-sided Fisher increment significant vs BOTH generic-vs-parent rates (vs option_value p=**0.0021**; vs floor p=**0.0186**) AND H2H vs floor 27/33 = 0.818. **Honest caveat:** direct H2H vs the *stronger* generic option_value scorer is `no_edge` (23/39 = 0.590, CI spans 0.5) — attribution rests on the increment + floor H2H, not a clean direct win over the strongest baseline. Noise self-mirrors clean.
- **Decision `diamond_specialist_promising_local_only`** — a LOCAL signal justifying a larger-N rematch, NOT a promotion/prod change (human approval required first). LOCAL-ONLY; references benchmark-only & **excluded** from the eval; no upload/promote/register/tick/republish/redeploy; all five workflows stayed not-started; shared site **not** regenerated; 10 LOCAL `no_upload` events (clean). **135** Pass-46J tests + pass41–46i regression green.
- Next: confirm at N≥60–80 seat-balanced (especially vs the generic option_value scorer) to try to convert the directional H2H into a confirmed edge; strengthen the `search_to_hand`/`setup_bench` policies; only then, with human approval, consider a prod candidate path behind the existing safety + promotion gates.

Detail: `data/reports/pass46j_diamond_specialist_planner_report.md`; `docs/DIAMOND_SPECIALIST_PLANNER_V0.md`; `data/experiments/pass46j_*`.

## Pass 46K — Diamond specialist larger-N confirmation + reference gap audit v0 (LOCAL-ONLY, NO redeploy) — decision: diamond_specialist_inconclusive_needs_more_n
- Re-evaluated the **existing** 46J candidate `cg_typed_diamond_specialist_planner_v0` at larger N by **reusing the 46J/46H tarballs byte-for-byte** (no rebuild, no overwrite). 399 games total (229 carried from 46J + 170 new; 3 invalid); carried and new rows reported separately, never silently pooled.
- **Both gating arms now CLEAR — the planner edge is real and attributable.** Practical `spec_vs_parent` = 61/84 = **0.726**, Wilson95 [0.623, 0.810] → `confirmed_edge`; both generic 46H scorers still lose to the same parent (ov 0.438, floor 0.500). Attribution `spec_vs_generic_ov` — directional-only in 46J — now **clears** at 53/84 = **0.631** [0.524, 0.726] → `confirmed_edge` over the *stronger* generic scorer, plus a significant one-sided Fisher increment over both generic-vs-parent rates (vs ov p=**0.000993**; vs floor p=**0.007981**) and floor H2H 0.797. The parent edge is the planner's, not inherited from the 46H generic family.
- **Blocked by an unclean noise control, not by the edge.** The triggered `spec_vs_spec` self-mirror (identical agent vs itself, should be ~0.5 by symmetry) sits at **11/34 = 0.324**, Wilson [0.191, 0.492] excluding 0.5, persistently across both tranches **and both seats** (0.333/0.313) — most plausibly a cabt seat-order asymmetry the subject-label doesn't track. `parent_vs_parent` mirror is clean (0.588, CI ∋ 0.5). The frozen ladder therefore holds the larger-N readout as **inconclusive** rather than confirming through an unclean apparatus — goalposts not moved.
- **Decision `diamond_specialist_inconclusive_needs_more_n`** (`confirmed_local_candidate=false`), provably **independent of references**. LOCAL-ONLY; public references benchmark-only & **excluded** (pooled distance-from-parity **1/60 = 0.017** [0.003, 0.089], parity unsupported, Part-F games in their own no_upload ledger; ActiveGraph benchmark ledger untouched); no upload/promote/register/tick/republish/redeploy; all five workflows stayed not-started; shared report site **not** regenerated; 12 LOCAL `no_upload` events (clean, idempotent).
- Next: **diagnose the `spec_vs_spec` mirror** — re-run it **seat-paired** (same seed, swapped seats) at N≥60 to test the seat-order hypothesis; fold any seat correction into the noise-clean rule and re-derive. The practical + attribution arms already clear, so a clean mirror would put `confirmed_local_candidate` in reach **without** any new candidate build. Only then, with human approval, consider a prod candidate path behind the existing safety + promotion gates.

Detail: `data/reports/pass46k_diamond_specialist_confirmation_report.md`; `data/experiments/pass46k_*`.

## Pass 46L — Diamond reference-gap planner upgrade v1 (LOCAL-ONLY, NO redeploy) — decision: diamond_v1_not_promising
- Built ONE owned upgrade `cg_typed_diamond_specialist_planner_v1` from the existing 46J v0 via a **narrow visible-only structural diff** (stronger attach-to-active weighting, harsher already-energized penalty, most-energized promote tilt), targeting the 46K public-reference gap (v0 = W1/L59 pooled). Owned original `main.py`, deck **byte-identical** to the diamond parent, inline region byte-identical, behavioral parity vs the module — candidate well-formed (validation + fixtures all_ok; cg accepts / stdlib rejects unchanged; no reference code copied; root untouched).
- **Stage-2 non-inertness gate FAILED — v1 is definitively INERT.** Replaying v0 vs v1 on the SAME real frames: **0 / 312** changed decisions (rate 0.0; 138 attach-multi frames, 0 flips; 0 illegal; 0 empty-fallback). The visible-only levers move the attach active-vs-bench crossover only fractionally (base/penalty ratio ≈2.5→≈2.8) and v0's active-preference already dominates, so the argmax is unchanged on every frame that occurs. Per the frozen ladder, an inert v1 BLOCKS all eval panels → no reference panel was run (it would reproduce v0's W1/L59 by construction).
- **Decision `diamond_v1_not_promising`**, provably **independent of references** (forcing the reference test both ways leaves it unchanged; the inert gate short-circuits first). LOCAL-ONLY; public references benchmark-only & never gated; ActiveGraph benchmark ledger untouched; no upload/promote/register/tick/republish/redeploy; all five workflows stayed not-started; shared report site **not** regenerated; 5 LOCAL `no_upload` events (clean, idempotent on marker `pass46l_eventset`).
- **Durable lesson — the reference gap is a card-id GROUNDING gap, not a thin-policy gap.** 46K showed a role-KEYED visible layer fired too rarely; 46L shows a STRUCTURAL visible layer is outright inert. A second visible-only policy layer is now twice proven a dead end. Next: do NOT ship another visible-only layer — run a **card-id grounding pass** (ground the role/attacker/energy map to an authoritative OWNED card database, independently audited, NOT extended from observed play = invented IDs), or pivot to a different archetype whose winning line is expressible from visible state. Only then is a reference-gap improvement plausibly reachable, behind the existing safety + promotion gates with human approval.

Detail: `data/reports/pass46l_diamond_reference_gap_planner_report.md`; `data/experiments/pass46l_*`.
