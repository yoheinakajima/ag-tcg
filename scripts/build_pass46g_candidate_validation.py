#!/usr/bin/env python3
"""PASS 46G (Part F) — candidate validation + lane separation (all candidates).

Validates EVERY Part-E candidate WITHOUT running a tournament or touching prod:
  ...
  2. The stdlib lane REJECTS it (non-stdlib cg import + extra cg/ tree) — proving
     our own Kaggle-lane gates are unaffected by these benchmark-lane candidates.
  ...
  4. Lane separation / originality: each candidate's main.py and tarball hashes
     match NO public-reference main.py or tarball (zero-copy of reference policy),
     and the candidates are mutually distinct (distinct tarballs/decks/profiles).

LOCAL / READ-ONLY. No mutation, no upload, no events. Writes
data/experiments/pass46g_candidate_validation.{json,md}. Exit 0 iff all_ok.
"""
...
fuzz_inputs = [
    ...
    {"current": {"turn": 6, "deckCount": 4, "players": [{}, {}]},
     "select": {"option": [{"type": 13}, {"type": 8, "inPlayArea": 1}],
                "minCount": 1, "maxCount": 1}},
]
...
cases = [
    ...
    {"option": [{"type": 8, "inPlayArea": 1, "inPlayIndex": 0},
                {"type": 8, "inPlayArea": 2, "inPlayIndex": 1}, {"type": 14}],
     "minCount": 1, "maxCount": 1},
]
for sel in cases:
...
def main() -> int:
    ...
    data = {
        "pass": "46g", "part": "F", "read_only": True, "production_mutated": False,
    ...
    (EXP / "pass46g_candidate_validation.json").write_text(
        json.dumps(data, indent=2, default=str) + "\n", encoding="utf-8")

    md = [
        "# Pass 46G (Part F) — candidate validation + lane separation", "",
        "_LOCAL / READ-ONLY. No tournament, no upload, no events. Proves every "
        "candidate is accepted by the cg_typed lane, REJECTED by the stdlib lane "
        "(our own Kaggle gates untouched), never-raises on malformed input, always "
        "returns legal indices, copies NO public-reference policy, and that the "
        "candidates are mutually distinct._", "",
        ...
        f"{distinct_mains}",
        f"- **ALL OK:** **{all_ok}**", "",
        "| candidate | family | profile | cg accepts | stdlib rejects | deck | "
        "fuzz | legal | no-ref-match | ok |",
        "|---|---|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|",
    ]
    for r in results:
    ...
    (EXP / "pass46g_candidate_validation.md").write_text("\n".join(md) + "\n",
                                                         encoding="utf-8")
