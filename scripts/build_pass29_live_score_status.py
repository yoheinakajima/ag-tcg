#!/usr/bin/env python3
"""Pass 29 (Part B) — read-only live-score status. LOCAL / READ-ONLY.
...
THREE distinct fields per spec:
  ...
  * portfolio_reference      — the in-portfolio Water reference deck that the
                               internal league is calibrated around.

The live score is reported for CONTEXT only; this observability pass never gates
a build or a promotion. publicScore is coerced safely to float. Writes
data/experiments/pass29_live_score_status.{json,md} and (read-only) tee logs to
data/kaggle_uploads/status_before_pass29.{log,csv} when a live call succeeds.
"""
...
PASS28 = REPO / "data" / "experiments" / "pass28_live_score_status.json"
OUT_JSON = REPO / "data" / "experiments" / "pass29_live_score_status.json"
OUT_MD = REPO / "data" / "experiments" / "pass29_live_score_status.md"
...
def _try_live_listing() -> bool:
    ...
    if proc.returncode != 0 or not proc.stdout.strip():
        (KU / "status_before_pass29.log").write_text(
            (proc.stdout or "") + "\n" + (proc.stderr or ""), encoding="utf-8")
        return False
    (KU / "status_before_pass29.csv").write_text(proc.stdout, encoding="utf-8")
    (KU / "status_before_pass29.log").write_text(
        proc.stdout + "\n" + (proc.stderr or ""), encoding="utf-8")
    return True
...
def main() -> int:
    ...
    drift = []
    if PASS28.exists():
        p28 = json.loads(PASS28.read_text(encoding="utf-8"))
        prev = {r["fileName"]: r["publicScore"]
                for r in p28.get("complete_submissions_ranked", [])}
        ...
        for fn in sorted(set(prev) | set(cur)):
            ...
            drift.append({"fileName": fn, "pass28": pv, "pass29": cv, "delta": delta})
    ...
    status = {
        ...
        "promotion_note": ("Pass 29 is an OBSERVABILITY + effect-loop feasibility "
        ...
        "internal_league_caveat": (
            "The internal league (and these forensic traces) are NOT the Kaggle "
    ...
    L = ["# Pass 29 — Live Score Status (Part B, read-only)", "",
         "> Read-only Kaggle status. **No submit, no upload.** Observability pass — "
         "the live score is context only and gates NOTHING here. Three controls are "
         "kept explicitly distinct, and the live score is NOT the internal league.", "",
         ...
         "", "## Score drift vs Pass 28", "",
         "| fileName | pass28 | pass29 | delta |", "|---|---|---|---|"]
    for d in drift:
        L.append(f"| {d['fileName']} | {d['pass28']} | {d['pass29']} | {d['delta']} |")
