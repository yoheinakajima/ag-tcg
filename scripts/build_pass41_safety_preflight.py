#!/usr/bin/env python3
"""Pass 41 (Part A) — root / production / benchmark-lane safety preflight.

OPS / verify-only STOP-GATE. Re-confirms, WITHOUT mutating anything, the hard
invariants before the Pass 41 cg_typed candidate spike (a LOCAL candidate
generation/evaluation pass — NOT a Kaggle upload/submission pass):
...
Writes data/experiments/pass41_root_prod_benchmark_safety.{json,md} (and, via the
health checker, pass41_tournament_health.{json,md}). Exit nonzero if any hard
...
def _run_prod_health() -> dict:
    out_json = EXP / "pass41_tournament_health.json"
    out_md = EXP / "pass41_tournament_health.md"
    try:
        proc = subprocess.run(
...
def _scan_ledger_events() -> dict:
    """No forbidden upload/submit events and no event with no_upload=false."""
    forbidden = 0
    no_upload_false = 0
    scanned = 0
    per_file = {}
...
def main() -> int:
    ...
    payload = {
        "pass": "41", "part": "A", "local_only": True, "no_upload": True,
        "upload_performed": False, "auto_submit": False, "github_push": False,
        "candidate_generation": False, "tarball_mutation": False,
        ...
        "config_ok": config_ok,
        "root_prod_benchmark_safe": safe,
        "stop_required": not safe,
    }
    (EXP / "pass41_root_prod_benchmark_safety.json").write_text(
    ...
    dc = deploy["checks"]
    md = [
        "# Pass 41 — Root / production / benchmark-lane safety preflight (Part A)", "",
        "> OPS / verify-only STOP-GATE. Pass 41 is a LOCAL cg_typed candidate "
        "generation/evaluation pass. NO upload, NO submit, NO auto-submit, NO push, "
        "NO root/tarball mutation. Public references are benchmark-only — never our "
        "candidates, never lineage parents, never queued/promoted. Internal "
    ...
    md += [
        ...
        f"- no forbidden upload/submit events: **{yn(lsc['no_forbidden_events'])}** "
        ...
        "## Verdict",
        f"- root + prod + benchmark-lane safe: **{yn(safe)}**",
        f"- **stop required: {yn(not safe)}**", "",
        "## package verify-only output (tail)", "```", *verify_tail, "```",
    ]
    (EXP / "pass41_root_prod_benchmark_safety.md").write_text(
        "\n".join(md) + "\n", encoding="utf-8")

    print(f"pass41 preflight: safe={safe} root_unchanged={root_unchanged} "
