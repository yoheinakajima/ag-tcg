#!/usr/bin/env python3
"""Pass 28 (Part B) — read-only live-score status. LOCAL / READ-ONLY.
...
THREE distinct fields per the Pass-28 spec:
  ...
  * portfolio_reference      — the in-portfolio Water reference deck that the
                               Pass-27/28 internal league is calibrated around.

The live score is reported for CONTEXT only; this diagnostic pass never gates a
build or a promotion. publicScore is coerced safely to float. Writes
data/experiments/pass28_live_score_status.{json,md} and (read-only) tee logs to
data/kaggle_uploads/status_before_pass28.{log,csv} when a live call succeeds.
"""
...
PASS27 = REPO / "data" / "experiments" / "pass27_live_score_status.json"
OUT_JSON = REPO / "data" / "experiments" / "pass28_live_score_status.json"
OUT_MD = REPO / "data" / "experiments" / "pass28_live_score_status.md"
...
def _try_live_listing() -> bool:
    ...
    if proc.returncode != 0 or not proc.stdout.strip():
        (KU / "status_before_pass28.log").write_text(
            (proc.stdout or "") + "\n" + (proc.stderr or ""), encoding="utf-8")
        return False
    (KU / "status_before_pass28.csv").write_text(proc.stdout, encoding="utf-8")
    (KU / "status_before_pass28.log").write_text(
        proc.stdout + "\n" + (proc.stderr or ""), encoding="utf-8")
    return True
...
def main() -> int:
    ...
    drift = []
    if PASS27.exists():
        p27 = json.loads(PASS27.read_text(encoding="utf-8"))
        prev = {r["fileName"]: r["publicScore"]
                for r in p27.get("complete_submissions_ranked", [])}
        ...
        for fn in sorted(set(prev) | set(cur)):
            ...
            drift.append({"fileName": fn, "pass27": pv, "pass28": cv, "delta": delta})
    ...
    status = {
        ...
        "read_only": True, "upload_performed": False, "is_promotion_decision": False,
        "promotion_note": ("Pass 28 is a cross-deck FORENSIC / diagnostic pass. The "
                           "live score is context only and never gates a build or a "
        ...
        "portfolio_reference": _fld(
            pref, "in-portfolio Water reference deck the internal league is "
        ...
        "distinction_note": (
            ...
            "other two."),
        "internal_league_caveat": (
            "The Pass-27/28 INTERNAL league (and these forensic traces) are NOT the "
            "Kaggle leaderboard: both seats are our own decks driven by the same "
            "generic pilot. Internal win rates and live publicScore are different "
            "measurements and must never be conflated."),
        "score_drift_vs_pass27": drift,
    ...
    L = ["# Pass 28 — Live Score Status (Part B, read-only)", "",
         "> Read-only Kaggle status. **No submit, no upload.** Forensic pass — the "
         "live score is context only and gates NOTHING here. Three controls are kept "
         "explicitly distinct, and the live score is NOT the internal league.", "",
         ...
         "", "## Score drift vs Pass 27", "",
         "| fileName | pass27 | pass28 | delta |", "|---|---|---|---|"]
    for d in drift:
        L.append(f"| {d['fileName']} | {d['pass27']} | {d['pass28']} | {d['delta']} |")
    ...
    L += ["", "## Notes", "", f"- {status['promotion_note']}", "",
          ...
          f"- {status['internal_league_caveat']}", ""]
