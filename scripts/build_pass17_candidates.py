#!/usr/bin/env python3
"""Pass 17 (Part E) -- build the internal-league deck candidates.
...
wrapper, last-callable entrypoint). Per deck we:
  ...
  * build the 60-card deck.csv from experiments/deck_ideas.yaml (validated ids only);
  ...
  * load the deck's pass17 playbook (card roles only);
...
OUT_ROOT = REPO / "data" / "submissions" / "candidates_pass17"
RUNS_ROOT = REPO / "experiments" / "runs_pass17"
OVERRIDE_MARKER = "# === PASS14 CORE-PILOT OVERRIDE:"

# Tarballs of decks we can reuse verbatim by `deck_source`.
SOURCE_TARBALLS = {
...
def extract_base_main(v2_main: str) -> str:
    """v2's deck-safe base main.py (everything before the override block)."""
...
def swap_embedded_deck(base_main: str, deck_ids: list[int]) -> str:
    """Replace the base's deck-safety block so _EMBEDDED_DECK is THIS deck."""
...
def build_one(key: str, spec: dict) -> dict:
    ...
    if deck_source:
        ...
        # Water reference: keep the base's embedded deck (already this deck).
    else:
...
def main() -> int:
    ...
    manifest = {"pass": "17", "part": "E", "local_only": True,
