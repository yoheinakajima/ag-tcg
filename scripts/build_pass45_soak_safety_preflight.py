#!/usr/bin/env python3
"""PASS 45 — Part A: production probation SOAK safety preflight (STOP-GATE).

Re-asserts every hard safety invariant BEFORE any read-only soak audit of the live
Scheduled Deployment, and folds in the post-republish evidence (the 3 Pass-42
tarballs exist locally, sha-match the Pass-42 manifest, and are git-tracked at HEAD,
and remain local-probation). It consumes the refreshed
``pass43_safety_preflight.json`` (Pass-43 Part A re-run against the live prod
snapshot) for the shared hard checks plus the prod-health verdict.

Read-only. NO upload/submit/push/root-mutation/generation/promotion/tarball-deletion.
NEVER starts the root "Start application" workflow. STOP if any hard check fails.
Output: data/experiments/pass45_soak_safety_preflight.{json,md}
"""
...
HEALTH43 = EXP / "pass43_tournament_health.json"
...
def _tarball_checks() -> dict:
    ...
    tracked = set(_git("ls-files", "--",
                       "data/submissions/generated_pass42/").splitlines())
    ...
    all_ok = bool(rows) and all(
        r["exists_local"] and r["sha_match"] and r["git_tracked"]
        and r["is_probation"] for r in rows)
...
def _replit_storage_backend_check() -> dict:
    ...
    uses_root_main = '\nrun = ["python", "main.py"]' in txt
    return {"replit_app_storage": uses_app_storage,
            "deployment_tick": uses_tick, "production_flag": uses_production,
            "deployment_not_root_main": not uses_root_main,
            "all_ok": uses_app_storage and uses_tick and uses_production
            and not uses_root_main}


def _health_freshness() -> dict:
    if not HEALTH43.is_file():
        return {"present": False}
    h = json.loads(HEALTH43.read_text(encoding="utf-8"))
    mtime = HEALTH43.stat().st_mtime
    return {"present": True, "mtime": mtime,
            "healthy": h.get("healthy"),
            "hard_failures": h.get("hard_failures"),
            "warnings": h.get("warnings"),
            "manifest_sha_integrity_checked":
                "manifest_sha_integrity" not in (h.get("hard_failures") or [])}
...
def main() -> int:
    ...
    if not PRE43.is_file():
        raise SystemExit("missing pass43_safety_preflight.json — run Pass-43 Part A "
                         "(re-run, fresh prod health) first")
    pre = json.loads(PRE43.read_text(encoding="utf-8"))

    # fresh, filecmp-derived root immutability (never inherited/hardcoded)
    root_main_fresh = filecmp.cmp(REPO / "main.py", BASELINE / "main.py",
                                  shallow=False)
    root_deck_fresh = filecmp.cmp(REPO / "deck.csv", BASELINE / "deck.csv",
                                  shallow=False)
    root_unchanged_fresh = bool(root_main_fresh and root_deck_fresh)
    root_unchanged = root_unchanged_fresh and bool(pre.get("root_unchanged"))
    ...
    health_fresh = _health_freshness()
    ...
    out = {
        "pass": "pass45_partA_soak_safety_preflight",
        ...
        "deck_mutation_performed": False,
        "head_commit": tarballs["head_commit"],
        "root_immutability_fresh": {
            "main_py_byte_identical": root_main_fresh,
            "deck_csv_byte_identical": root_deck_fresh,
            "filecmp_derived": True,
            "cross_check_pass43": bool(pre.get("root_unchanged")),
        },
        "checks": checks,
        "replit_storage_backend_check": storage,
        "tarball_availability": tarballs,
        "prod_health_freshness": health_fresh,
    ...
    (EXP / "pass45_soak_safety_preflight.json").write_text(
    ...
    md = [
        "# PASS 45 — Part A: production probation SOAK safety preflight (STOP-GATE)",
        "",
        "> Read-only OPS gate re-asserted BEFORE any soak audit of the live Scheduled "
        "Deployment. NO upload, NO submit, NO push, NO root/tarball mutation, NO "
        "candidate generation, NO deck mutation, NO promotion. The root 'Start "
        "application' workflow is NEVER started. Internal diagnostics; NOT a Kaggle "
        "leaderboard or strength claim.", "",
    ...
    md += [
        ...
        f"- health artifact present: **{yn(health_fresh.get('present'))}**  "
        f"healthy: **{yn(health_fresh.get('healthy'))}**",
        f"- hard failures: {health_fresh.get('hard_failures') or 'none'}  "
        f"warnings: {health_fresh.get('warnings') or 'none'}",
        f"- deep manifest sha integrity verified: "
        f"**{yn(health_fresh.get('manifest_sha_integrity_checked'))}**",
        "", "## Root 'Start application' workflow",
        "- frozen Kaggle entrypoint; not-started is EXPECTED and is **never** started "
        "by this pass.", "",
        "## Shared Pass-43 preflight (re-run, fresh prod health)",
    ...
    (EXP / "pass45_soak_safety_preflight.md").write_text(
        "\n".join(md) + "\n", encoding="utf-8")

    print(f"pass45 partA preflight: safe={safe} stop_required={not safe} "
