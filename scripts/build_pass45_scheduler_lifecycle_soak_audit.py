#!/usr/bin/env python3
"""PASS 45 — Part F: scheduler / lifecycle SOAK audit (read-only).

Rebuilds the scheduler worklist from the PRODUCTION ledger exactly as the deployed
daemon would (fold events -> rankings -> scheduler state -> worklist) and re-asserts
every lifecycle invariant after the production soak:

  * the 3 Pass-42 probation candidates are schedulable AND placed in the worklist;
  ...
  * protected statuses (family_champion / portfolio_anchor / held_probe) are present
    and were not demoted by any ledger event;
  * no active-cap replacement / demotion was forced;
  * no promotion / retirement / forbidden events exist in the prod ledger.

Projection writes are redirected to a temp dir (no local pollution). Production is
never mutated. Output:
  data/experiments/pass45_scheduler_lifecycle_soak_audit.{json,md}
"""
...
def _load_prod():
    ...
    tmp = Path(tempfile.mkdtemp(prefix="pass45_sched_prod_")) / "events.jsonl"
...
def main() -> int:
    ...
    projmod.PROJ_DIR = Path(tempfile.mkdtemp(prefix="pass45_proj_"))
    ...
    targets_probation = sorted(t for t in targets
                               if status_of.get(t) == poolmod.PROBATION)
    ...
    out = {
        "pass": "pass45_scheduler_lifecycle_soak_audit",
        "all_ok": all_ok,
        "checks": checks,
    ...
    (EXP / "pass45_scheduler_lifecycle_soak_audit.json").write_text(
    ...
    (EXP / "pass45_scheduler_lifecycle_soak_audit.md").write_text(
        "\n".join(md) + "\n", encoding="utf-8")

    print(f"pass45 partF: all_ok={all_ok} worklist={len(wl1)} "
