#!/usr/bin/env python3
"""PASS 46H (Part A) — safety stop-gate for the within-family per-option value sprint.

LOCAL / verify-only STOP-GATE. Pass 46H is a LOCAL gameplay-improvement sprint that
builds a SMALL BATCH of owned cg_typed turn-planner candidates (<=6) whose hot path
adds WITHIN-FAMILY per-option value features on top of the Pass-46G family scorer. It
is NOT a Kaggle upload/submission/promotion/registration pass and performs NO
production tick / republish. This preflight freshly re-confirms, WITHOUT mutating
anything, the hard invariants before any candidate is built:
  ...
  7. the Pass 46D primitives + 46E oracle/calibration + 46F label dataset artifacts are
     present (the inputs this sprint reuses);
  8. the Pass 46G outputs this sprint extends are present (turn_planner_profiles scorer
     module, profile catalog, candidate build record, owned candidate tarballs, and the
     eval panel) — 46H is the WITHIN-FAMILY follow-up to 46G's TOP-1-INERT phase/role
     layer, so it must not run without 46G's measured baseline on disk.

Writes data/experiments/pass46h_safety_preflight.{json,md}. Exit nonzero if unsafe.
...
CAND_46G_DIR = REPO / "data" / "submissions" / "candidates_pass46g"
...
# 46G outputs this WITHIN-FAMILY follow-up extends / measures against.
REQUIRED_46G = [
    REPO / "src/ptcg_activegraph/analysis/turn_planner_profiles.py",
    EXP / "pass46g_profile_catalog.json",
    EXP / "pass46g_profile_calibration.json",
    EXP / "pass46g_candidate_build.json",
    EXP / "pass46g_eval_panel.json",
]
...
def _g46_tarballs() -> list[str]:
    return sorted(Path(p).name for p in glob.glob(str(CAND_46G_DIR / "*.tar.gz")))
...
def main() -> int:
    ...
    missing_46g = sorted(str(p.relative_to(REPO)) for p in REQUIRED_46G if not p.exists())
    traces_present = sorted(Path(p).name for p in glob.glob(str(TRACES / "*.json.gz")))
    g46_tarballs = _g46_tarballs()
    ...
    checks = {
        ...
        "pass46g_artifacts_present": missing_46g == [],
        "pass46g_candidate_tarballs_present": len(g46_tarballs) > 0,
        "pass46c_traces_present": len(traces_present) > 0,
    }
    all_ok = all(checks.values())

    data = {
        "pass": "46h", "part": "A", "read_only": True, "production_mutated": False,
        ...
        "missing_46g_artifacts": missing_46g,
        "n_pass46g_candidate_tarballs": len(g46_tarballs),
        "pass46g_candidate_tarballs": g46_tarballs,
        "n_traces_present": len(traces_present),
        "checks": checks,
        "all_ok": all_ok,
        "stop_required": not all_ok,
    }
    (EXP / "pass46h_safety_preflight.json").write_text(
    ...
    md = [
        "# Pass 46H — Part A: safety stop-gate", "",
        "> LOCAL gameplay-improvement sprint (WITHIN-FAMILY per-option value turn-planner "
        "candidates, the follow-up to 46G's TOP-1-INERT coarse phase/role layer). NOT a "
        "Kaggle upload/submission/promotion/registration pass. No prod Object Storage "
        "mutation, no tick, no republish, no upload/submit, no auto-submit, no "
    ...
    md += [
        ...
        f"- 46G candidate tarballs present: {len(g46_tarballs)}",
        ...
        f"- missing 46G artifacts: {missing_46g or 'none'}", "",
        f"**all_ok = {all_ok}** — **stop_required = {not all_ok}**",
    ]
    (EXP / "pass46h_safety_preflight.md").write_text("\n".join(md) + "\n",
                                                     encoding="utf-8")
