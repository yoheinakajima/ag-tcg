#!/usr/bin/env python3
"""PASS 46F (Part A) — safety stop-gate for the search-calibrated turn-planner pilot.

LOCAL / verify-only STOP-GATE. Pass 46F is a LOCAL gameplay-improvement pass that
builds ONE owned cg_typed candidate pilot. It is NOT a Kaggle upload/submission/
promotion pass. This preflight freshly re-confirms, WITHOUT mutating anything, the
hard invariants before any candidate is built:
  ...
  7. the Pass 46D primitives + Pass 46E oracle/calibration artifacts are present
     (the inputs this pass depends on).

Writes data/experiments/pass46f_safety_preflight.{json,md}. Exit nonzero if unsafe.
...
# Inputs this pass strictly depends on.
...
def main() -> int:
    ...
    data = {
        "pass": "46f", "part": "A", "read_only": True, "production_mutated": False,
    ...
    (EXP / "pass46f_safety_preflight.json").write_text(
    ...
    md = [
        "# Pass 46F — Part A: safety stop-gate", "",
        "> LOCAL gameplay-improvement pass (search-calibrated turn-planner candidate "
        "pilot). NOT a Kaggle upload/submission/promotion pass. No prod Object Storage "
    ...
    (EXP / "pass46f_safety_preflight.md").write_text("\n".join(md) + "\n",
                                                     encoding="utf-8")
