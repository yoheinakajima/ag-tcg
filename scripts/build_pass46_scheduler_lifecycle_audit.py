#!/usr/bin/env python3
"""PASS 46 — scheduler / lifecycle read-only audit.
...
every lifecycle invariant after the soak:
...
never mutated. Output:
  data/experiments/pass46_scheduler_lifecycle_audit.{json,md}
"""
...
def _load_prod():
    ...
    tmp = Path(tempfile.mkdtemp(prefix="pass46_sched_prod_")) / "events.jsonl"
...
def main() -> int:
    ...
    projmod.PROJ_DIR = Path(tempfile.mkdtemp(prefix="pass46_proj_"))
    ...
    out = {
        "pass": "pass46_scheduler_lifecycle_audit",
        "all_ok": all_ok,
        "checks": checks,
    ...
    (EXP / "pass46_scheduler_lifecycle_audit.json").write_text(
    ...
    (EXP / "pass46_scheduler_lifecycle_audit.md").write_text(
        "\n".join(md) + "\n", encoding="utf-8")

    print(f"pass46 scheduler: all_ok={all_ok} worklist={len(wl1)} "
