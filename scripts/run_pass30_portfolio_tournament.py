#!/usr/bin/env python3
"""Pass 30 (Part G) — internal existing-portfolio tournament. LOCAL ONLY.

THIS IS NOT A KAGGLE LEADERBOARD. Every participant is one of OUR portfolio decks
(or an evidence-backed variant of one) piloted by the SAME generic core pilot;
opponents are our own decks, not real Kaggle policies. These win rates measure
deck/pilot COMPATIBILITY internally and do NOT predict Kaggle standings.

Participants = the Part-F tournament-eligible candidates (validators pass, smoke
clean, NOT blocked_from_league). Durant is EXCLUDED by design (deckout/mill win
condition the generic pilot cannot pilot; its smoke is INVALID).

Two phases:
  * STAGE 1 (default): full round-robin, P30_GAMES_PER_SEAT (default 3) games with
    each deck seat 0 then seat 1 (seat swap cancels first-player bias).
  * STAGE 2 (P30_STAGE=2): focused round-robin among the Stage-1 top 4 at
    P30_GAMES_PER_SEAT (default 10), written to *_stage2.{json,md}.

Games run IN-PROCESS via _MEV._run_game (kaggle_environments imported once),
capturing behavioral metrics (first attack/evolution step, attack count) that are
robustly derivable from cabt logs; deeper board metrics are honestly reported as
not_measured. RESUMABLE: each call plays as many matchups as fit in
P30_PER_CALL_BUDGET_S, persisting crash-safe progress. Re-invoke until
"remaining=0".

Outputs (data/experiments/): pass30_portfolio_tournament.{json,md},
pass30_portfolio_matrix.csv, pass30_portfolio_rankings.{json,md} (+ *_stage2.*).
...
STAGE = int(os.environ.get("P30_STAGE", "1"))
GAMES_PER_SEAT = int(os.environ.get("P30_GAMES_PER_SEAT", "10" if STAGE == 2 else "3"))
GAME_TIMEOUT_S = int(os.environ.get("P30_GAME_TIMEOUT_S", "60"))
PER_CALL_BUDGET_S = int(os.environ.get("P30_PER_CALL_BUDGET_S", "95"))
TOP_N = int(os.environ.get("P30_TOP_N", "4"))

SUFFIX = "_stage2" if STAGE == 2 else ""
PROGRESS = EXP / f"pass30_portfolio_tournament{SUFFIX}_progress.json"
RANKINGS_STAGE1 = EXP / "pass30_portfolio_rankings.json"

DISCLAIMER = (
    "INTERNAL TOURNAMENT — NOT A KAGGLE LEADERBOARD. All participants are our own "
    "portfolio decks (or evidence-backed variants) piloted by the same generic core "
    "pilot; opponents are not real Kaggle policies. Win rates measure internal "
    "deck/pilot compatibility only and do not predict Kaggle results. No candidate "
    "is uploaded or submitted.")

# Family map for labelling / family aggregation.
FAMILY = {
...
REFERENCE_IDS = {"league_water_core_reference", "league_water_anti_disruption_pivot_v1",
                 "core_pilot_water_v2_runtime"}
DIAGNOSTIC_IDS = {"league_mega_charizard_x_burst", "league_mega_gardevoir_psychic_ramp"}
LOOP_GUARD_IDS = {"effect_loop_exit_guard_v1"}
...
def _run_game(agent_a: str, agent_b: str) -> dict:
    # Honor this pass's per-game timeout via the shared runner.
...
def _play_matchup(a_id, a_agent, b_id, b_agent, per_seat) -> dict:
    ...
    # Behavioral metrics for A (from A's seat each game), best-effort.
...
def _participants_stage1() -> tuple[list[dict], list[str]]:
    ...
    if not VALIDATION.exists():
        return parts, ["Part-F validation report missing; cannot confirm eligibility"]
    val = json.loads(VALIDATION.read_text(encoding="utf-8"))
    elig = val.get("tournament_eligible") or val.get("league_eligible") or []
    meta = {r["candidate_id"]: r for r in val.get("candidates", [])}
    for cid in elig:
        m = meta.get(cid, {})
        parts.append({"id": cid, "family": FAMILY.get(cid, m.get("family")),
                      "role": m.get("role")})
    for r in val.get("candidates", []):
        if r.get("blocked_from_league"):
            notes.append(f"EXCLUDED {r['candidate_id']}: blocked_from_league "
                         f"({r.get('block_reason') or 'by design'})")
    return parts, notes
...
def _participants_stage2() -> tuple[list[dict], list[str]]:
    ...
    standings = rk.get("standings", [])
    top = standings[:TOP_N]
    parts = [{"id": r["id"], "family": r.get("family"), "role": r.get("role")}
             for r in top]
...
def _init_progress() -> dict:
    ...
    return {
        "pass": "30", "part": "G", "stage": STAGE, "local_only": True,
...
def _label(r: dict) -> str:
    ...
    if cid in LOOP_GUARD_IDS:
        return "candidate_for_confirmation" if awr >= 0.45 else "fixed_but_not_competitive"
...
def build_standings(prog: dict) -> dict:
    ...
    for p in prog["participants"]:
        agg[p["id"]] = {"id": p["id"], "family": p.get("family"),
                        "role": p.get("role"), "wins": 0, "losses": 0, "draws": 0,
                        "games": 0, "invalids": 0, "timeouts": 0, "crashes": 0}
    ...
    return {"pass": "30", "stage": STAGE, "disclaimer": prog["disclaimer"],
...
def _md_tournament(prog: dict, done: bool) -> str:
    L = [f"# Pass 30 — internal portfolio tournament (Part G, stage {prog['stage']})",
         "", f"> {prog['disclaimer']}", "",
    ...
    L += ["", "_Behavioral metrics (first attack/evolution step, attack count) are "
          "best-effort from cabt logs; deeper board metrics (no-bench/deckout, "
          "effect-loop count) are not robustly decodable and are reported elsewhere "
          "as not_measured._", ""]
    return "\n".join(L)


def _md_rank(rank: dict) -> str:
    L = [f"# Pass 30 — portfolio tournament standings (Part G, stage {rank['stage']})",
         "", f"> {rank['disclaimer']}", "",
...
def _save_outputs(prog: dict, done: bool) -> None:
    ...
    (EXP / f"pass30_portfolio_tournament{SUFFIX}.json").write_text(
        json.dumps({**prog, "status": "complete" if done else "in_progress",
                    "matchups": list(prog["matchups"].values())},
                   indent=2, default=str), encoding="utf-8")
    (EXP / f"pass30_portfolio_tournament{SUFFIX}.md").write_text(
        _md_tournament(prog, done), encoding="utf-8")
    if prog["matchups"]:
        rank = build_standings(prog)
        (EXP / f"pass30_portfolio_rankings{SUFFIX}.json").write_text(
            json.dumps(rank, indent=2, default=str), encoding="utf-8")
        (EXP / f"pass30_portfolio_rankings{SUFFIX}.md").write_text(
            _md_rank(rank), encoding="utf-8")
        _write_matrix(prog, EXP / f"pass30_portfolio_matrix{SUFFIX}.csv")
...
def main() -> int:
    ...
    sentinel = EXP / f"pass30_portfolio_tournament{SUFFIX}.DONE"
    if sentinel.exists():
        sentinel.unlink()
