#!/usr/bin/env python3
"""PASS 46L (Part E) — build the ONE owned cg_typed Diamond SPECIALIST PLANNER candidate.
...
``deck.csv`` + ``cg/``:
  ...
  - main.py  : an ORIGINAL deck-specific TURN PLANNER whose hot path is the VERBATIM
               ``diamond_specialist_v1.py`` INLINE_DIAMOND_SPECIALIST_V1 region (no ``src/``
...
``src/ptcg_activegraph/analysis/diamond_specialist_v1.py``; this generator embeds its INLINE
...
to a NEW ``candidates_pass46l/`` dir), NO public reference as source/parent, NO reference
...
from ptcg_activegraph.analysis import diamond_specialist_v1 as DS  # noqa: E402
...
OUT_DIR = SUBM / "candidates_pass46l"
CG_FILES = ["__init__.py", "api.py", "game.py", "sim.py", "utils.py", "libcg.so"]
CAND_ID = "cg_typed_diamond_specialist_planner_v1"
...
OWNED PASS-46L Diamond SPECIALIST TURN PLANNER. The hot path is the VERBATIM
``diamond_specialist_v1.py`` INLINE_DIAMOND_SPECIALIST_V1 region: it builds a visible-only
...
def build() -> dict:
    ...
    if not region or "INLINE_DIAMOND_SPECIALIST_V1_BEGIN" not in region:
        raise SystemExit("could not extract INLINE_DIAMOND_SPECIALIST_V1 region")
    ...
    rec = {
        "pass": "46l", "part": "E", "read_only": False, "local_only": True,
        ...
        "planner_parent_candidate_id": "cg_typed_diamond_specialist_planner_v0",
        "planner_variant": "narrow_visible_only_structural_v1",
        ...
        "planner_source_module":
            "src/ptcg_activegraph/analysis/diamond_specialist_v1.py",
...
def _write_md(rec: dict) -> None:
    p = rec["parity"]
    lines = [
        "# PASS 46L · Part E — Owned cg_typed Diamond Specialist Planner candidate", "",
    ...
    (EXP / "pass46l_candidate_build.md").write_text("\n".join(lines) + "\n",
                                                    encoding="utf-8")
...
def main() -> int:
    ...
    (EXP / "pass46l_candidate_build.json").write_text(
