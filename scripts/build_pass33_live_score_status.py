#!/usr/bin/env python3
"""Pass 33 (Part B) — read-only live-score status. LOCAL / READ-ONLY.

Attempts a READ-ONLY Kaggle submissions listing (no submit, no upload). Order of
attempts: Python KaggleApi (the CLI is not on PATH in this repl) -> `kaggle` CLI
-> last-known-good live-score registry snapshot. publicScore is coerced safely to
float; only complete, scored rows feed the rankings.

Keeps FOUR distinct fields per the Pass 33 spec, computed independently:
  * live_score_leader         — highest complete publicScore in the listing.
  * water_family_current_best — the maintained Water-lineage anchor's score.
  * dragapult_family_best     — the best Dragapult-family submission's score.
  * portfolio_reference       — the in-portfolio Water reference the internal
                                tournament is calibrated around.

This pass makes NO promotion decision from the live score. Whether Dragapult is
above or below Water is read from THIS fresh listing, never hardcoded.
Writes data/experiments/pass33_live_score_status.{json,md} and (read-only) tee
logs to data/kaggle_uploads/status_before_pass33.{log,csv} on a live read.
"""
...
PREV = REPO / "data" / "experiments" / "pass32_live_probe_comparison.json"
OUT_JSON = REPO / "data" / "experiments" / "pass33_live_score_status.json"
OUT_MD = REPO / "data" / "experiments" / "pass33_live_score_status.md"
...
def _tee_rows(rows: list[dict]) -> None:
    """Write a read-only CSV + log snapshot of the live listing."""
    ...
    (KU / "status_before_pass33.csv").write_text(buf.getvalue(), encoding="utf-8")
    (KU / "status_before_pass33.log").write_text(
...
def main() -> int:
    ...
    # Drift vs Pass 32 comparison artifact (filename -> public_score).
    drift = []
    if PREV.exists():
        prev_doc = json.loads(PREV.read_text(encoding="utf-8"))
        prev = {c["filename"]: c.get("public_score")
                for c in prev_doc.get("candidates", [])}
        ...
        for fn in sorted(set(prev) | set(cur)):
            ...
            drift.append({"fileName": fn, "pass32": pv, "pass33": cv, "delta": delta})
    ...
    status = {
        ...
        "promotion_note": ("Pass 33 is a deck-composition STRESS TEST. The live "
                           "score is context only and never gates a build or a "
        ...
        "distinction_note": (
            ...
            "separate fields. A higher-scoring non-Water submission becomes "
            "live_score_leader WITHOUT changing the others."),
        "internal_tournament_caveat": (
            "The internal tournament in this pass is NOT the Kaggle leaderboard: "
            "both seats are our own decks driven by the same generic pilot. "
            "Internal win rates and live publicScore are different measurements "
            "and must never be conflated."),
        "score_drift_vs_pass32": drift,
    ...
    L = ["# Pass 33 — Live Score Status (Part B, read-only)", "",
         "> Read-only Kaggle status. **No submit, no upload.** The live score is "
         "context only and gates NOTHING here. Four controls are kept explicitly "
         "distinct, and the live score is NOT the internal tournament.", "",
         ...
         "", "## Score drift vs Pass 32", "",
         "| fileName | pass32 | pass33 | delta |", "|---|---|---|---|"]
    for d in drift:
        L.append(f"| {d['fileName']} | {d['pass32']} | {d['pass33']} | {d['delta']} |")
    ...
    print(f"dragapult_family_best: {dr['fileName'] if dr else None} @ "
          f"{dr['publicScore'] if dr else None}")
