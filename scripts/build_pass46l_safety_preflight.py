#!/usr/bin/env python3
"""PASS 46L (Part A) — safety stop-gate for the Diamond reference-gap planner UPGRADE.

LOCAL / verify-only STOP-GATE. Pass 46L builds ONE upgraded owned candidate
``cg_typed_diamond_specialist_planner_v1`` from the EXISTING Pass-46J candidate
``cg_typed_diamond_specialist_planner_v0`` (forked logic, NOT its bytes), targeting
public-reference FAILURES (46K: v0 beat parent/generic_ov/floor internally but went 1/60
pooled vs 5 public references). Production keeps soaking — this pass performs NO production
tick / republish / candidate registration / lifecycle / status apply / promotion, and NO
Kaggle upload/submit/auto-submit. Public references stay benchmark-only opponents (never
source/parent/candidate, never a decision gate).
...
This preflight freshly re-confirms, WITHOUT mutating anything, the hard invariants:
  ...
  6. the source parent ``diamond_toolbox_diancie`` tarball exists and is INTERNAL — it is
     NOT a public reference and NOT in a never-schedule status; the deck for v1 is copied
     byte-for-byte from this parent;
  7. the Pass-46J v0 candidate tarball (the forked logic source) is present, and the v0
     planner module ``src/ptcg_activegraph/analysis/diamond_specialist.py`` is present;
  8. the Pass-46J AND Pass-46K Diamond specialist artifacts we carry baselines / compare
     against are present (eval plans/panels/games, edge analysis, the 46K public-reference
     gap whose pooled W1/L59 is the FROZEN v0 reference baseline, strategy decisions);
  9. the Pass-46H generic ``option_value`` scorer tarball (the attribution baseline for the
     v1_vs_generic_ov panel) is present, and the card DB CSV is present;
 10. the NEW output dir ``candidates_pass46l`` holds no FOREIGN tarball (re-runnable: only
     the expected v1 tarball name is permitted next to where we will write).

Writes data/experiments/pass46l_safety_preflight.{json,md}. Exit nonzero if unsafe.
...
# --- Source / candidate-under-build / comparison artifacts -------------------------------
...
# Pass-46J v0 candidate — the FORKED LOGIC SOURCE (read-only; not rebuilt).
...
V0_PLANNER_MODULE = REPO / "src" / "ptcg_activegraph" / "analysis" / "diamond_specialist.py"

# Pass-46L NEW owned candidate output dir + the one permitted tarball name.
CAND_46L_DIR = REPO / "data" / "submissions" / "candidates_pass46l"
CAND_46L_ID = "cg_typed_diamond_specialist_planner_v1"
CAND_46L_TARBALL = CAND_46L_DIR / f"{CAND_46L_ID}.tar.gz"

# Pass-46H attribution baseline (generic option-value scorer) for the v1_vs_generic_ov panel.
CAND_46H_DIR = REPO / "data" / "submissions" / "candidates_pass46h"
DIAMOND_46H_OV = CAND_46H_DIR / "cg_typed_diamond_option_value_v1.tar.gz"

# Pass-46J + Pass-46K Diamond artifacts: baselines we carry / compare against. The 46K
# public-reference gap pooled W1/L59 is the FROZEN v0 reference baseline for Part I/J.
...
REQUIRED_46K_ARTIFACTS = [
    EXP / "pass46k_eval_plan.json",
    EXP / "pass46k_edge_analysis.json",
    EXP / "pass46k_public_reference_gap.json",
    EXP / "pass46k_strategy_decision.json",
]
...
def _v0_source_check(prod: dict) -> dict:
    """Confirm the forked logic source (46J v0) is present, INTERNAL, and not never-schedule."""
    try:
        ref_ids = set(promotion.load_reference_ids())
    except Exception:  # noqa: BLE001
        ref_ids = set()
    in_refs = CAND_46J_ID in ref_ids
    if prod.get("read_ok"):
        status = (prod.get("diamond_parent_status") if CAND_46J_ID == DIAMOND_PARENT_ID
                  else _local_pool_status(CAND_46J_ID))
    else:
        status = _local_pool_status(CAND_46J_ID)
    return {
        "v0_candidate_id": CAND_46J_ID,
        "v0_tarball": str(CAND_46J_TARBALL.relative_to(REPO)),
        "v0_tarball_present": CAND_46J_TARBALL.exists(),
        "v0_tarball_sha256": _sha(CAND_46J_TARBALL),
        "v0_in_reference_ids": in_refs,
        "v0_planner_module_present": V0_PLANNER_MODULE.exists(),
        "v0_status_not_never": status is None or status not in NEVER,
    }


def _output_dir_safe() -> tuple[bool, list[str]]:
    """Re-runnable: only a FOREIGN tarball (not the expected v1) makes the output dir unsafe.

    The dir may not exist yet (fine — Part E creates it); if it exists, only the expected
    v1 tarball name is permitted (a prior re-run's v1 is fine; the build never overwrites
    differing bytes)."""
    if not CAND_46L_DIR.exists():
        return True, []
    foreign = sorted(p.name for p in CAND_46L_DIR.glob("*.tar.gz")
                     if p.name != CAND_46L_TARBALL.name)
...
def _frozen_reference_baseline() -> dict:
    """Read (do NOT recompute) the FROZEN v0 reference baseline from the 46K gap artifact."""
    out = {"present": False, "wins": None, "losses": None, "decisive": None,
           "win_rate": None, "source": "pass46k_public_reference_gap.json"}
    p = EXP / "pass46k_public_reference_gap.json"
    if p.exists():
        try:
            pooled = json.loads(p.read_text(encoding="utf-8")).get("pooled", {})
            out.update({"present": True, "wins": pooled.get("wins"),
                        "losses": pooled.get("losses"), "decisive": pooled.get("decisive"),
...
def main() -> int:
    ...
    v0src = _v0_source_check(prod)
    out_dir_safe, foreign_tarballs = _output_dir_safe()
    frozen_baseline = _frozen_reference_baseline()

    missing_46j = sorted(str(p.relative_to(REPO)) for p in REQUIRED_46J_ARTIFACTS
                         if not p.exists())
    missing_46k = sorted(str(p.relative_to(REPO)) for p in REQUIRED_46K_ARTIFACTS
                         if not p.exists())
    ...
    checks = {
        ...
        "v0_source_tarball_present": v0src["v0_tarball_present"],
        "v0_source_is_internal_not_reference": v0src["v0_in_reference_ids"] is False,
        "v0_planner_module_present": v0src["v0_planner_module_present"],
        "pass46j_diamond_artifacts_present": missing_46j == [],
        "pass46k_diamond_artifacts_present": missing_46k == [],
        "frozen_reference_baseline_present": frozen_baseline["present"],
        "pass46h_option_value_tarball_present": DIAMOND_46H_OV.exists(),
        "card_db_csv_present": CARD_DB_CSV.exists(),
        "candidates_pass46l_no_foreign_tarballs": out_dir_safe,
    }
    all_ok = all(checks.values())

    data = {
        "pass": "46l", "part": "A", "read_only": True, "production_mutated": False,
        ...
        "root_main_unchanged": main_ok, "root_deck_unchanged": deck_ok,
        ...
        "v0_source": v0src,
        "candidate_to_build_id": CAND_46L_ID,
        "candidate_to_build_tarball": str(CAND_46L_TARBALL.relative_to(REPO)),
        "candidates_pass46l_dir": str(CAND_46L_DIR.relative_to(REPO)),
        "candidates_pass46l_foreign_tarballs": foreign_tarballs,
        "frozen_reference_baseline": frozen_baseline,
        "pass46h_option_value_tarball": str(DIAMOND_46H_OV.relative_to(REPO)),
        "missing_46j_artifacts": missing_46j,
        "missing_46k_artifacts": missing_46k,
        "card_db_csv": str(CARD_DB_CSV.relative_to(REPO)),
        "checks": checks,
        "all_ok": all_ok,
        "stop_required": not all_ok,
    }
    (EXP / "pass46l_safety_preflight.json").write_text(
    ...
    md = [
        "# Pass 46L — Part A: safety stop-gate", "",
        "> LOCAL reference-gap UPGRADE pass: build ONE upgraded owned candidate "
        "`cg_typed_diamond_specialist_planner_v1` from the EXISTING Pass-46J "
        "`cg_typed_diamond_specialist_planner_v0` (forked logic, NOT its bytes), targeting "
        "public-reference failures (46K: v0 = 1/60 pooled vs 5 refs, the FROZEN baseline). "
        "Production keeps soaking. NO prod Object Storage mutation, no tick, no republish, no "
        "candidate registration, no lifecycle/status apply, no promotion, no Kaggle "
        "upload/submit, no auto-submit, no CandidatePromoted/SubmissionQueued/"
        "SubmissionUploaded/KaggleScoreUpdated, no public reference as source/parent/"
        "candidate (refs benchmark-only opponents, never a decision gate), no root "
        "main.py/deck.csv mutation, deck.csv copied byte-identical from the parent, no "
    ...
    md += [
        ...
        f"- v0 logic source: `{v0src['v0_candidate_id']}` present="
        f"{v0src['v0_tarball_present']}, planner_module_present="
        f"{v0src['v0_planner_module_present']}",
        f"- FROZEN v0 reference baseline (46K): wins={frozen_baseline['wins']}, "
        f"losses={frozen_baseline['losses']}, decisive={frozen_baseline['decisive']}, "
        f"win_rate={frozen_baseline['win_rate']}",
        f"- candidate to build: `{CAND_46L_ID}` (output dir foreign tarballs: "
        f"{foreign_tarballs or 'none'})",
        f"- missing 46J artifacts: {missing_46j or 'none'}",
        f"- missing 46K artifacts: {missing_46k or 'none'}", "",
        f"**all_ok = {all_ok}** — **stop_required = {not all_ok}**",
    ]
    (EXP / "pass46l_safety_preflight.md").write_text("\n".join(md) + "\n",
                                                     encoding="utf-8")
