#!/usr/bin/env python3
"""PASS 45 — Part E: internal promotion gate v1 — PRODUCTION evidence DRY-RUN ONLY.
...
``events.jsonl``, never the ~950 sidecars). This is a DRY-RUN ONLY pass: it emits
nothing, mutates nothing, and NEVER calls the gate's ``--apply`` path. No
CandidateStatusChanged / CandidatePromoted / Kaggle events under any circumstance.

The expected honest outcome is insufficient_evidence for the 3 Pass-42 probation
candidates: the production soak has accrued only a handful of placement games, far
below the locked v1 sample-size minimums.

Output:
  data/experiments/pass45_promotion_gate_prod_dry_run.{json,md}
"""
...
def _fast_prod_eval() -> dict | None:
    ...
    try:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="pass45_gate_prod_")) / "events.jsonl"
        ...
        print(f"[pass45-gate] prod fast-eval unavailable: {exc}")
        return None
...
def main() -> int:
    ...
    # honesty: in a DRY-RUN-ONLY pass NO target may carry an actionable rec
    ...
    out = {
        "pass": "pass45_promotion_gate_prod_dry_run",
        "mode": "dry_run_only",
        "apply_invoked": False,
        "emitted_events": 0,
        "mutated": False,
    ...
    (EXP / "pass45_promotion_gate_prod_dry_run.json").write_text(
    ...
    md = [
        "# PASS 45 — Part E: promotion gate v1 — production DRY-RUN ONLY", "",
    ...
    (EXP / "pass45_promotion_gate_prod_dry_run.md").write_text(
        "\n".join(md) + "\n", encoding="utf-8")

    print(f"pass45 partE gate dry-run: local_actionable={local_eval['n_actionable']} "
