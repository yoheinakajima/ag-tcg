#!/usr/bin/env python3
"""PASS 46L (Part F) — candidate validation + lane separation (single specialist planner).
...
data/experiments/pass46l_candidate_validation.{json,md}. Exit 0 iff all_ok.
"""
...
BUILD_JSON = EXP / "pass46l_candidate_build.json"
PREFLIGHT_JSON = EXP / "pass46l_safety_preflight.json"
...
PLANNER_PARENT_TARBALL = ROOT / "data" / "submissions" / "candidates_pass46j" / \
...
def distinct_from_planner_parent(tar: Path) -> dict:
    """Informational: the v1 candidate's main.py must DIFFER from the v0 planner parent
    (this is a v0->v1 upgrade pass — the artifact really changed). Not part of all_ok;
    the behavioural v0-vs-v1 gate is the Part-G non-inertness test."""
    with tarfile.open(tar, "r:gz") as t:
        cand_main = t.extractfile("main.py").read()
    if not PLANNER_PARENT_TARBALL.exists():
        return {"planner_parent_present": False,
                "candidate_main_sha256": _sha(cand_main),
                "distinct_from_planner_parent_v0": "unverified"}
    with tarfile.open(PLANNER_PARENT_TARBALL, "r:gz") as t:
        v0_main = t.extractfile("main.py").read()
    return {"planner_parent_present": True,
            "planner_parent_id": "cg_typed_diamond_specialist_planner_v0",
            "candidate_main_sha256": _sha(cand_main),
            "planner_parent_main_sha256": _sha(v0_main),
            "distinct_from_planner_parent_v0": cand_main != v0_main}
...
def main() -> int:
    ...
    planner_parent = distinct_from_planner_parent(tar)
    root = root_unchanged()
    ...
    data = {
        "pass": "46l", "part": "F", "read_only": True, "production_mutated": False,
        ...
        "deck_vs_parent": deck,
        "distinct_from_planner_parent_v0": planner_parent,
        "root_unchanged": root,
        "all_ok": all_ok,
    }
    EXP.mkdir(parents=True, exist_ok=True)
    (EXP / "pass46l_candidate_validation.json").write_text(
        json.dumps(data, indent=2, default=str) + "\n", encoding="utf-8")

    bs = data["behavior_summary"]
    md = [
        "# PASS 46L · Part F — candidate validation + lane separation", "",
        ...
        f"- distinct from planner parent v0 (informational, not a gate): "
        f"**{planner_parent.get('distinct_from_planner_parent_v0')}**", "",
    ...
    (EXP / "pass46l_candidate_validation.md").write_text("\n".join(md) + "\n",
                                                         encoding="utf-8")
